...
N = 100
xt = simXt(N = N, mu = 0, numTrend = 2, numFreq = 2)$Xt
plot(xt, type = 'l'); grid()
...
lines(MtP + TtTmp, type = 'l', col = 'red')
lines(MtP + TtTmp + simWt(N = 100, var = 0.1)$value, type = 'l', col = 'green')



## Function to estimate linear trend component
estimate_linear_Mt <- function(x){
  N = length(x)
  Mt = estimateMt(x = x, N = N, nw = 5, k = 8, pMax = 2)
  centr = length(Mt) / 2
  mu = mean(Mt)
  z = 1:N; a = coef(lm(Mt ~ z))[2] * N
  return(simMt(N = N, numTrend = 1, a = a, mu = mu, center = center)$value)
}
...
t = 0:(N-1)
mu = 0
x = simXt(N = N, mu = mu, numTrend = numTrend)
xt = x$Xt
mt = x$Mt
x_fun = x$fn

plot(xt, type = 'l'); grid()
MtP = estimateMt(x = xt, N = N, nw = 5, k = 8, pMax = 2)
lines(MtP, col = 'blue', lwd = 0.5)
...
lines(Mt, type = 'l', col = 'green', lwd = 0.5)
