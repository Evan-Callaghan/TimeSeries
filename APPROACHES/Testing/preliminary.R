...
model = create_model(N, units = 256, connected_units = 32, activation = 'relu')

NNI_int = main(X0, max_iter = 1, train_size = 256, model = autoencoder, epochs = 100, batch_size = 32)

plot(NNI_int, type = 'l', col = 'green'); grid()
lines(X0)
...
x0 = interpTools::simulateGaps(list(X), p = c(0.1, 0.2), g = c(10, 20), K = 3)

MODELS = c(create_model(N = 1000, units = 64, connected_units = 32, 'relu'))
TRAIN_SIZE = c(160, 320)
EPOCHS = c(10, 20)
BATCH_SIZE = c(32)

int = simulation_impute(x0, MODELS, TRAIN_SIZE, EPOCHS, BATCH_SIZE)
int

plot(int$model1$ts320$e20$bs32$p0.2$g20[[3]], type = 'l', col = 'green'); grid()
lines(x0[[1]]$p0.2$g20[[3]])
