...
4. Impose a gap structure on the simulated time series (i.e., the same gap structure as 
   observed in the original time series) and produce K unique ‘gappy’ time series (inputs).
...
```{r setup3, message=FALSE, warning=FALSE, fig.align='center', fig.dim=c(6,4)}
...
```{r demo1, fig.align='center', fig.dim=c(6,4)}
...
```{r demo2, fig.align='center', fig.dim=c(6,4)}
...
```{r demo3, fig.align='center', fig.dim=c(10,6), echo = FALSE}
step_3_4_demo <- function(x0, x, w){
  
  options(warn=-1)
  step3 = simulator_demo(x0, x, w, n_series = 3, p = 0.1, g = 2, K = 1, random = TRUE)
  inputs = step3[[1]]; targets = step3[[2]]
  
  par(mfrow = c(1,1))
  plot(xt, type = 'l', main = 'Demo Time Series (Random = TRUE)', lwd = 2); grid()
  ...
  lines(targets[3,], type = 'l', col = 'dodgerblue', lwd = 0.5)
  lines(which(is.na(inputs[3,])), targets[3, is.na(inputs[3,])], type = 'p', col = 'black', 
        pch = 21, bg = 'dodgerblue', cex = 0.7)
  
  return(list(inputs, targets))
}

step3_4 = step_3_4_demo(x_gappy, step2_x, step2_w)
```

```{r demo3b, fig.align='center', fig.dim=c(10,6), echo = FALSE}
step_3_4_demo <- function(x0, x, w){
  
  options(warn=-1)
  step3 = simulator_demo(x0, x, w, n_series = 3, p = 0.1, g = 2, K = 1, random = FALSE)
  inputs = step3[[1]]; targets = step3[[2]]
  
  par(mfrow = c(1,1))
  plot(xt, type = 'l', main = 'Demo Time Series (Random = FALSE)', lwd = 2); grid()
  lines(which(is.na(x_gappy)), xt[is.na(x_gappy)], type = 'p', col = 'black', pch = 21, bg = 'black')
