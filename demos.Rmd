...
## New Notes:
A Fourier Series approximates any given function using a summation of sine and cosine functions with unique frequencies.
A Fourier Transform allows use to go between the time domain and the frequency domain (specifically a time domian signal into a frequency domain signal), and an inverse Fourier transform does the opposite.
...
```{r demo3, fig.align='center', fig.dim=c(6,10)}
