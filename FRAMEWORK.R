...
results = FRAMEWORK(X, P = c(0.1, 0.2), G = c(10, 20), K = 5, METHODS = c('HWI', 'LI', 'EWMA', 'NNI'))
results


## Simple plot of aggregated RMSE across methods
interpTools::plotMetrics(results, p = 0.1, g = 10, metric = 'RMSE')

## Producing 
multiHeatmap(crit = 'RMSE', agEval = results, m = c('HWI', 'LI', 'EWMA', 'NNI'), by = 'method')
